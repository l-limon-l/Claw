        ...
        let showQuestPicker = function(quests) {
          return new Promise((resolve) => {
            ...
            const incomplete = quests.filter(
              (q) => !q.userStatus?.completedAt && new Date(q.config?.expiresAt).getTime() > Date.now() && q.id !== CONST.ID && !Tasks.skipped.has(q.id)
            );
          ...
          while (RUNTIME.running) {
            try {
              ...
              const active = quests.filter(
                (q) => !q.userStatus?.completedAt && new Date(q.config?.expiresAt).getTime() > Date.now() && q.id !== CONST.ID && !Tasks.skipped.has(q.id) && (!RUNTIME.selectedQuests || RUNTIME.selectedQuests.has(q.id))
              );
